...
import com.trainee.appinventiv.cryptocurrency.databinding.TopCurrencyLayoutBinding
...
class TopMarketAdapter(var context: Context, val list : List<CryptoCurrency>) : RecyclerView.Adapter<TopMarketAdapter.TopMarketViewHolder>() {
    ...
    override fun onBindViewHolder(holder: TopMarketViewHolder, position: Int) {
        ...
        holder.binding.topCurrencySymbolTextView.text = item.symbol

        holder.binding.topCurrencyPriceTextView.text = "${String.format("$%.02f", item.quotes[0].price)}"
